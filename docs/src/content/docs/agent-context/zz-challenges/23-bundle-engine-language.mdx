---
title: "Challenge 23: Bundle engine implementation language — TypeScript in-plugin vs external Python vs hybrid, with realistic 5-year survival, distribution, and platform-portability stress tests"
description: "Ch 20 settled what the import primitive should LOOK like (graph-aware ETL, ~5–6 algebraic primitives over a closed Tier-1 sink vocabulary, MTT-justified). Ch 21 settled the build-vs-buy meta-question (Compose: bundled engine + external producers + marketplace). This challenge asks the question both deferred: in WHAT LANGUAGE / RUNTIME does the bundled engine actually live? Pure in-plugin TypeScript (~480 KB; runs in Obsidian renderer)? External Python CLI (matches Ch 21's Polars+DuckDB stack; user installs Python)? Hybrid (small TS engine for tree-shaped sources in-plugin; external Python or other for advanced cases)? Or something else (Rust→WASM, Go→WASM, JVM)? Critically assess all paths against bundle-size budget, install friction, debuggability, ecosystem maturity, mobile-Obsidian portability, agent-author ergonomics, governance/sustainability, 5-year drift risk."
tags: [research, foundation, import, etl, runtime, language-choice, governance, sustainability, bundle-size, mobile-portability]
date: 2026-05-04
sidebar:
  label: "23 · Bundle engine language"
---
