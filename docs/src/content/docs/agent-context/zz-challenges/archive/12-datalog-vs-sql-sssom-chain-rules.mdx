...
date: 2026-05-02
sidebar:
  label: "Ch 12 · Datalog vs SQL chain rules"
  order: 1012
---

:::tip[Resolved 2026-05-02 — archived]
Resolved by two convergent fresh-agent deliverables ([Ch 12a](/crosswalker/agent-context/zz-research/2026-05-02-challenge-12-deliverable-a-datalog-vs-sql/), [Ch 12b](/crosswalker/agent-context/zz-research/2026-05-02-challenge-12-deliverable-b-beyond-engine-landscape/)) on Datalog (Nemo) primary + OxO2 architecture. Committed in the v0.1 stack pivot. Brief preserved as originally written.
:::

:::note[Rerun filed 2026-05-08 — Ch 36]
This challenge's narrow framing (Datalog vs SQL for SSSOM chain rules only) was sufficient for the 2026-05-02 verdict but doesn't engage the broader question — what query language do recipe authors / users *write*? Under the [ontology-web framing](/crosswalker/concepts/ontology-web-querying/) and the [3-layer architecture](/crosswalker/concepts/query-primitives/), the language question spans recipe YAML, codeblock body, view config, and inter-tier protocol. **A fresh rerun of this challenge has been filed as [Ch 36](/crosswalker/agent-context/zz-challenges/36-query-language-rerun/)** — original brief stays archived; the rerun produces a NEW deliverable that may revise or override the verdicts above.
:::
...
This is a **direct fork in the road**. The choice ripples into the Tier 2/3 engine selection ([Challenge 11](/crosswalker/agent-context/zz-challenges/archive/11-tier-2-3-engine-deep-survey/)) and into the audit-trail story (a clean `derivation_path` field per derived edge requires the engine to expose its rule-firing trace).
...
Challenge 08 (and the eventual [Challenge 13](/crosswalker/agent-context/zz-challenges/archive/13-modern-attestation-primitives/) follow-on) require every derived edge to carry an auditable `derivation_path`:
...
- Specific Datalog engine selection (Nemo vs Soufflé vs Datomic) — covered by [Challenge 11](/crosswalker/agent-context/zz-challenges/archive/11-tier-2-3-engine-deep-survey/)
...
- **Narrows from [Challenge 10](/crosswalker/agent-context/zz-challenges/archive/10-graph-to-tabular-bridging-engine/)**: Ch 10 picked recursive CTE without considering Datalog seriously
- **Coordinates with [Challenge 11](/crosswalker/agent-context/zz-challenges/archive/11-tier-2-3-engine-deep-survey/)**: Ch 11 picks the engine; Ch 12 picks the *paradigm* the engine must support
- **Independent of [Challenge 13](/crosswalker/agent-context/zz-challenges/archive/13-modern-attestation-primitives/)**: different layer

## Related

- [Challenge 10: Graph→tabular bridging engine](/crosswalker/agent-context/zz-challenges/archive/10-graph-to-tabular-bridging-engine/) — predecessor; recursive-CTE-only treatment
- [Challenge 11: Tier 2/3 engine deep survey](/crosswalker/agent-context/zz-challenges/archive/11-tier-2-3-engine-deep-survey/) — coordinates
