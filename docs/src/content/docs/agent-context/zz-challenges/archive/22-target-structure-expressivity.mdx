---
title: "Challenge 22: Target-structure expressivity in import recipes (archived)"
description: "RESOLVED 2026-05-04: closed grammar of 5 mechanisms (folder/file/heading/tag/wikilink) × ordered layout × also_emit × graph_edges; render(Recipe, ConceptIdentity) → Address as single coupling point; content addressing computed BEFORE render (target structure is a view); managed/user_preserve frontmatter split; v0.1 ships full schema with folder+file+heading wired. See callout."
tags: [research, foundation, import, target-structure, polyhierarchy, seacow, obsidian, information-science, archived]
date: 2026-05-03
sidebar:
  label: "22 · Target-structure (archived)"
---

import { Aside } from '@astrojs/starlight/components';

:::tip[Resolved 2026-05-04 — archived]
This challenge has been **resolved** and the brief is **archived for reference**. An adversarial fresh-agent deliverable settled the design:

- [Ch 22 deliverable: Target-structure expressivity](/crosswalker/agent-context/zz-research/2026-05-04-challenge-22-target-structure-expressivity/) — closed grammar of 5 mechanisms (`folder | file | heading | tag | wikilink`) × ordered `layout` × `also_emit` cross-cutting × optional `graph_edges`. Single coupling point: `render(Recipe, ConceptIdentity) → Address` modeled directly on RML/R2RML's `rr:subjectMap` + `rr:template`. Pass 1 vault-independent (deterministic, hashable); Pass 2 optional vault-state-aware link minimizer. Content addressing computed over canonical concept-identity store BEFORE render — target structure is a *view*, not canonical state — directly mirroring Nix's file-system-object/store-path separation. Frontmatter split into `managed` (recipe-owned) and `user_preserve` (user annotations, never overwritten). 4-phase non-breaking migration from existing `hierarchy` column-role.

The deliverable is synthesized in the **[2026-05-04 target-structure synthesis log](/crosswalker/agent-context/zz-log/2026-05-04-target-structure-synthesis/)** which adopts the full grammar at v0.1 (schema-paid-for-now, implementation-incremental) with `mechanism: folder | file | heading` wired in v0.1 and `tag | wikilink` as layout-levels deferred to v0.2.
