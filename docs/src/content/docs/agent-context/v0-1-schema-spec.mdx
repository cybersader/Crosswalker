...
**Stats**: `GenerationResult.edgeCount` = parent links + children entries + facet-hub member entries — the review screen's "K graph edges" line (a run shipping 0 edges is a visible warning, not a silent dead graph).

**Preset defaults** (stamped by `instantiate` onto `ImportMapping.enrichment`, then serialized to `target.enrichment`): `browsable-framework` → `children_lists: true`, `facet_notes: "notes"`. The hyperconnected default IS the default.
...
**Scope**: recipe + preset in this round — the workbench UI knobs for enrichment and the review-screen edge count land in a follow-up UX round. Folder-note relocation (step 2 of the design) is a separate chunk.
