...
:::note[Substrate choice — open research]
The current v0.1 substrate commitment is `@sqlite.org/sqlite-wasm` (canonical SQLite, foundation-governed). [Challenge 24](/crosswalker/agent-context/zz-challenges/24-turso-libsql-evaluation/) opened 2026-05-04 evaluates whether to migrate Tier 2 to **libSQL** (Turso's SQLite fork, native vector search, native replication) or stay on canonical. The DDL below is substrate-agnostic if libSQL's SQLite-compat claim holds; the *decision* is which substrate ships v0.1. Tracking this here so future readers know the choice is under active evaluation, not silently committed.
:::
