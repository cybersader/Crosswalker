...
| `parent_note` | `sibling` \| `folder-note` | `sibling` | Where a concept that is also a parent lives. `sibling`: `X.md` beside `X/` (v0.1). `folder-note`: `X/X.md` — schema-accepted but **falls back to sibling** at render time with a deviation note (relocation ships in a later chunk). |
