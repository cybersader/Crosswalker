...
sidebar:
  ...
  order: YYYYMMDD                   # numeric sort keeps chronological order
---
```

The `sidebar.label` shows a compact date in the nav so entries are scannable. The `order` field ensures chronological sorting (newest at bottom by default).
