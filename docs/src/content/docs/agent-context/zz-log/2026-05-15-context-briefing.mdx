...
| File | Role |
|---|---|
| [`query-frontmatter-schema.ts`](https://github.com/cybersader/Crosswalker/blob/main/src/views/query-frontmatter-schema.ts) | JSON Schema (2020-12) + AJV validator + `newQueryId()` + `viewFileFor()`. Validates the `crosswalker_query:` block at every read + write boundary. |
...
| [`regenerate-query-views.ts`](https://github.com/cybersader/Crosswalker/blob/main/src/views/regenerate-query-views.ts) | Vault scanner. `regenerateAll(app)` walks all markdown files; for each one with `crosswalker_query:` frontmatter, regenerates the `.base` file. Idempotent — skips when YAML body matches. Runs on plugin load (stale-state recovery) + as the explicit `Crosswalker: Refresh query views` command. |
