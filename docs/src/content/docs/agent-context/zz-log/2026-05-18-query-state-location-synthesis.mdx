...
| 2026-05-18 | Ch 38 filed — stress-test Phase 4.5 query state location | [Brief](/crosswalker/agent-context/zz-challenges/38-query-state-location-and-folder-note-pattern/) |
...
| 1.4 | **Slug is display; `query_id` (existing `q-YYYY-MM-DD-<8hex>`) is durable identity.** Rename-safe by keying scanner + writers on `query_id`, not folder path. | ✅ |
...
| Collision policy | Always `--<id8>` suffix on folder | Refuse-and-prompt (picker) + `-<4hex>` (programmatic) | **B+ mixed policy** |
...
| 7 | Programmatic create (agent / future MCP / wizard-draft) | Append `-<4hex>` random suffix. Never silently `-2`/`-3`. | Avoids the brief's flagged anti-pattern ("user can't find auto-renamed query"). Suffix is detectable. |
...
- `query_id` format (`q-<YYYY-MM-DD>-<8hex>`)
- AJV validation
...
| Collision policy | Picker modal tests for case 6 (3 options surface); programmatic tests for case 7 (`-<4hex>` suffix added) |
...
- [ ] Programmatic create produces `-<4hex>` suffix when slug collides
...
| Slug-collision policy | ❓ open | ✅ **Locked: refuse-and-prompt (picker) + `-<4hex>` (programmatic)** |
...
- **Ch 38 brief**: [Where does query state live? Stress-test the folder-note pattern.](/crosswalker/agent-context/zz-challenges/38-query-state-location-and-folder-note-pattern/) — archived after this log lands
