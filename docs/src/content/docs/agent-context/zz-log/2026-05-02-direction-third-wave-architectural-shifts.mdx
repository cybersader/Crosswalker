...
### 7.2 Confirmed defaults pending explicit user sign-off

- **Tier 3 default flip** (this log §3) — confirm Fuseki primary + oxigraph-server alternative + AGE as fallback. **[Confirm? Y/N]**
- **Tier 1 audit 4-tier model + T2 OTS default** (this log §4) — confirm T0/T1/T2/T3 model + OpenTimestamps as new T2 default + git stack as one of three T3 options. **[Confirm? Y/N]**
- **Tier 2 stack confirmation + Tier 2-Lite + Comunica add-on** (this log §2) — confirm Tier 2-Lite alternate stack + Comunica federation. **[Confirm? Y/N]**
...
| **Ch 18 candidate (Tier 2-Lite SSSOM rule subset)** | Worth spinning up; pending user signal | Ch 14 explicitly flags missing scope definition for which SSSOM rules work under recursive-CTE evaluation and the scale ceiling |
