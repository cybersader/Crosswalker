...
| **Tier 2 substrate** | `@sqlite.org/sqlite-wasm` (canonical, foundation-governed) vs libSQL (Turso's fork — native vector, native replication, single-vendor) | Stay on canonical sqlite-wasm unless Ch 24 surfaces a specific reason to migrate | Open — [Ch 24](/crosswalker/agent-context/zz-challenges/24-turso-libsql-evaluation/) spun up 2026-05-04 to evaluate against bundle size, mobile portability, single-vendor governance, SQLite-compat divergence, vector-search angle, marketplace pattern alignment |
...
| 7 | **Recipe DSL spec — machine-readable** | `spec/recipe.schema.json` | UNBLOCKED 2026-05-04 by [Ch 23 resolution](/crosswalker/agent-context/zz-log/2026-05-04-bundle-engine-language-synthesis/). Runtime-agnostic JSON Schema; JSONata 2.x as expression sub-language; reserve `producer` field for v0.5+. The single most important architectural commitment per Ch 23 §4 |
