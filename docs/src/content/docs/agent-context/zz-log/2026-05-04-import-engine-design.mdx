...
| Open | Options | Default lean |
|---|---|---|
| **Bundled engine implementation language** | TypeScript/Bun (in-plugin); Python (external CLI); both | Both — tiny TS in-plugin reference engine for zero-config UX; Python CLI for power users / CI / agents. The "external connections, doesn't need to be Obsidian-internal" framing applies. |
| **Recipe DSL surface syntax** | YARRRML-shaped YAML; Dhall-typed; JSON Schema with discriminated unions; hybrid | YARRRML-shaped YAML (familiar to data-engineering audiences; mature toolchain) |
| **Recipe storage location** | `.crosswalker/recipes/` in vault; plugin user-data dir; user choice | `.crosswalker/recipes/` in vault — git-versions with content; matches files-canonical principle |
| **Marketplace mechanism** | GitHub repo (v0.1); built-in registry (v1.0+); Obsidian community plugins (v1.0+) | GitHub repo at v0.1 (simplest); built-in registry once usage justifies it |
| **Target_schema as data vs hardcoded** | Hardcoded (v0.1 simplicity); declarative dialect spec (longer-term) | Hardcoded for v0.1 reference engine; declarative dialect spec for v1.0+ when community variants emerge |
| **Architectural fork resolution from new Ch 21 deliverable** (Python+Polars+DuckDB external recommendation) | Adopt for external tier; in-plugin TS for v0.1 reference; both validated | Validated: external Python is one valid producer (per §2.3); v0.1 reference engine stays TS/Bun in-plugin per the [v0.1 stack pivot](/crosswalker/agent-context/zz-log/2026-05-02-v0-1-initial-stack-pivot/) |
...
| 2 | **ETL-and-import concept pillar** | `concepts/etl-and-import` (new) | TODO — frames the schema-as-primitive + marketplace + tabular asymmetry |
| 3 | **Import engine design pillar** | `design/import-engine` (new — supersedes stale `design/transformation`) | TODO — the engine signature + recipe DSL + transformation catalog + reference implementation plan |
...
| 7 | **Recipe DSL spec — machine-readable** | `spec/recipe.schema.json` | TODO — the multi-axis selector |
