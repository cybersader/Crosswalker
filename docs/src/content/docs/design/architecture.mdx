...
For the high-level pipeline (Ingest → Parse → Transform → Generate → Link → Evolve), see **[The Pipeline](/crosswalker/concepts/pipeline/)** — the core architectural artifact that everything else hangs off of.
