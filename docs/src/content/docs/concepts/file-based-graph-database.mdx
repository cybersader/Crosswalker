...
[Obsidian Bases](/Crosswalker/concepts/metadata-ecosystem/) and Dataview function as **materialized views** over the file graph — they compute and display aggregated data from individual files. Unlike a SQL materialized view, they recompute on every query (no staleness risk, but no persistence either).
