...
Bases handles ~5 of 7 primitives natively for Layer B's table + list shapes. For pivot, graph, hierarchy, and timeline shapes (and for closure / anti-join primitives Bases can't express), Crosswalker provides an escape hatch via Tier 2 SQL helpers (already shipped v0.1.5 P3) and the upcoming `crosswalkerPivot` custom Bases view (v0.1.6).
