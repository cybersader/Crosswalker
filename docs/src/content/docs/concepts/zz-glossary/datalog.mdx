...
For Crosswalker at scale (10⁶+ mappings with branching factor over 5), the difference is real. At small scale (≤10⁵ mappings) recursive CTE is fine — which is why [Tier 2-Lite](/crosswalker/agent-context/zz-challenges/archive/18-tier-2-lite-sssom-rule-subset/) drops Datalog and uses recursive CTE instead.
...
- [Ch 18: Tier 2-Lite SSSOM rule subset and scale ceiling](/crosswalker/agent-context/zz-challenges/archive/18-tier-2-lite-sssom-rule-subset/) — defines which rules survive without Datalog
