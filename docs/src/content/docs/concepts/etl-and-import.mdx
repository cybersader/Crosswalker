...
## The three producer paths — and what's NOT a tier

A common misconception (clarified in [the 2026-05-05 ETL pipeline clarification log](/crosswalker/agent-context/zz-log/2026-05-05-etl-pipeline-clarification/)): there is **no static intermediate file format** between the source and Tier 1. The "normalized format Crosswalker excels at" *IS* Tier 1 itself.

Three independent ways to land valid Tier 1 — all first-class per the [schema-as-primitive commitment](#the-reframe--schema-is-the-primitive-etl-is-convenience):
...
Why this matters: the "external ETL" path doesn't transform a source into some Crosswalker-specific intermediate format that the engine then digests. It transforms the source **directly into Tier 1 Markdown**. The intermediate format you might imagine (a normalized CSV/JSON) doesn't exist because making it canonical *also* makes it require maintenance. Skipping it means the schema has *one* contract, not two.

### How Path A works inside the plugin (v0.1 implementation)

The bundled engine's per-row pipeline (after Ch 22 + Ch 23 + Ch 24 design phase, implemented in [v0.1.2](/crosswalker/reference/roadmap/milestones/v0-1-2-render/) + [v0.1.3](/crosswalker/reference/roadmap/milestones/v0-1-3-generation-engine-integration/) + [v0.1.4](/crosswalker/reference/roadmap/milestones/v0-1-4-junction-notes-and-crosswalks/)):
