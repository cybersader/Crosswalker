...
| **libSQL** (Turso) | Single-file (claims SQLite compat) | SQLite soft-fork; native vector + replication. **Under evaluation** — see [Ch 24](/crosswalker/agent-context/zz-challenges/archive/24-turso-libsql-evaluation/) |
...
| **Turso Database / Limbo** | Single-file (Rust rewrite of SQLite) | Pre-1.0; experimental. Long-horizon track via [Ch 24 Q3](/crosswalker/agent-context/zz-challenges/archive/24-turso-libsql-evaluation/) |
...
| **libSQL native vectors** | Lives inside libSQL single-file | Native vector type built into libSQL fork; the strongest specific case for migrating to libSQL per [Ch 24](/crosswalker/agent-context/zz-challenges/archive/24-turso-libsql-evaluation/) |
...
This is why the libSQL question ([Ch 24](/crosswalker/agent-context/zz-challenges/archive/24-turso-libsql-evaluation/)) is high-stakes despite the substrate being "another single-file SQLite-shaped thing" — single-vendor governance is exactly the dimension SQLite optimized away.
