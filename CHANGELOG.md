...
### Known defect, mostly closed: each import now gets its own folder, but an existing vault needs care (2026-08-28, amended 2026-08-29 and five times on 2026-08-30)
...
**The work to make it safe for a vault that already holds imports has now been attempted six times.** Each attempt was measured against a bar written down in advance rather than a checklist. Each has ended one level below the last, and at every level the thing that had to be removed was the same thing: something deciding a fact instead of reading one.

All six attempts rest on the same rule, which is the durable result of the work and is not in question: **refreshing a collection you already have never changes where that collection lives.** Its folder is read from the notes it already owns and then remembered, so a refresh has nothing to move and nothing to leave behind, and moving a framework somewhere else becomes something you ask for rather than something that happens to you.

**Who owns an import is settled and end-to-end verified. What a write is allowed to touch is now settled for identifiers and not for folders.** The first four attempts each tried to work out which collection an import belongs to and were each defeated by a case the previous one had not thought of; the fifth found the one place underneath all four that was guessing and deleted it. The sixth went one level lower again, to the part of the importer that places each note, which never consulted collections at all: it searched the whole vault for a note carrying the identifier it was about to write and used whatever it found, whoever owned it. That is now closed. Underneath *that*, still open, the same code asks the folder before it asks anything, and a folder does not name an owner.

What is true today, verified by 2,370 passing tests and by deliberately sabotaging each rule below in turn and watching its own tests fail, and, as of the previous attempt, by a full end-to-end run of 47 walkthroughs with no regression:
...
- **An import never writes over a note it does not own, when the two share an identifier.** Placing a note now looks only at the notes the collection doing the importing already owns. If your import would write an identifier a different collection holds, that row is refused and named: which identifier, which collection holds it, and where that note is. Nothing is written for it. It is not moved, not merged into, and not relabelled. Grouping and home notes follow the same rule.
- **The finish message states what moved and what is missing**, on every run, and says *orphans not checked* rather than *no orphans* when the check could not run. **Errors are always shown**, on a screen, rather than counted and closed over.
...
**None of it is offered here, because the same thing is still wrong one level further down, and it is the one that can cost you notes.**

- **An import can still write over a note it does not own, when the two share a folder rather than an identifier.** Placing a note asks the folder before it asks anything else. If a note is already sitting where the import is about to write, that note is used, and nothing reads who owns it. The two notes have different identifiers, so the rule above does not see it. The other collection's note is merged into and relabelled as this import's own, and on grouping notes it happens with nothing reported at all. At the other collection's next refresh the note is missing from what it owns, so it is written again and taken back. Nothing stops that repeating.
- **Two releases of one framework do not yet coexist.** A new collection whose identifiers would collide with an existing one is now created with its identifiers qualified, so the two no longer meet on every row. But the folder is still derived from the name of the file you imported, so two releases still land in one folder and now collide there instead, where nothing is watching. Qualifying the identifiers moved the collision from the place that reports it to the place that does not.
- **Answering the ownership question turns that qualification off.** It is only worked out when you never touch the ownership control. Both ways of asking for a new collection, the dropdown and the button that declines a suggested refresh, record your answer in a form that is returned before the collision is ever considered. So confirming the default on screen gives you unqualified identifiers, every row refused, and no notes written, while ignoring the screen entirely gives you the collection that works. That is exactly backwards.
- **The refusal applies everywhere and the way through it applies on two screens.** Refusing is done by the importer, so every path gets it. Qualifying is done by the import window and the mapping window, and nowhere else. The bundled-example command asks for a new collection with unqualified identifiers, so loading the same sample twice now produces a page of refusals: safe, and not what anyone wants to see. It also still reports how many errors happened without showing any of them, which is the one path left that closes over its own errors.
...
- **The end-to-end suite is red on this work.** Twenty-five walkthroughs fail and none of them times out; every failure is a real assertion. Sixteen of the seventeen new ones are internal test entry points importing the same source twice without naming a collection, which is precisely what the new rule refuses, so they are the test harness still importing the way the product no longer allows. It is still a red suite, and a red suite is not a verified one.
