...
### Known defect, mostly closed: each import now gets its own folder, but an existing vault needs care (2026-08-28, amended 2026-08-29 and twice on 2026-08-30)
...
**The work to make it safe for a vault that already holds imports has now been attempted three times, and is still not finished.** Each attempt was measured against a bar written down in advance rather than a checklist, and none of them has cleared it.

All three attempts rest on the same rule, which is the durable result of the work and is not in question: **refreshing a collection you already have never changes where that collection lives.** Its folder is read from the notes it already owns and then remembered, so a refresh has nothing to move and nothing to leave behind, and moving a framework somewhere else becomes something you ask for rather than something that happens to you. The code carrying that rule holds against 2,200 unit tests and causes no regression end to end: the full run is identical to the last one, test for test.

**The third attempt closed all four clauses the second one failed on, and a fresh review then found six more failures of the same bar.** What it closed: the ordinary wizard now labels an import with the name of the file it came from instead of the same placeholder every time, and the matching rule now ignores those placeholders entirely; a refresh that leaves your notes alone now counts them as present, so the grouping notes a collection owns are no longer reported as missing; the finish message now states what moved and what is missing on every run, not only on the runs where something happened; and an error now forces the summary screen open instead of closing the window over it.

**What the review found next, all of it the same defect wearing a different coat.**

- **Several of the bundled frameworks share one label.** Three bundled imports label their notes `nist-csf-2`, two label them `cis-v8`, and two more, including the general-purpose mapping import used for *every* pair of frameworks, label them `xwalk`. Import an ISO-to-NIST mapping and then a CSF-to-NIST mapping and the second is still offered as a refresh of the first. A label shared by unrelated imports is a placeholder wearing a real name, so replacing the obvious placeholders did not end the problem.
- **A saved configuration reused on two frameworks does the same thing.** Apply one saved import configuration to NIST and then to CIS and both are marked with that configuration's name, which the matching rule then reads as ownership. A configuration names reusable instructions, not the thing they were used on.
- **Two source files that happen to share a name still collide**, because the label is taken from the file name with no folder attached. `nist/controls.csv` and `cis/controls.csv` become the same label, and generic export names are common.
- **There is now no way to refresh a collection imported before this change.** Making old imports never match automatically is the safe default, but the "refresh that one instead" button is only ever drawn when something *did* match. Someone with an existing import is therefore offered nothing except "import as a new collection", which quietly writes a second copy of the same framework. The change removed an option it assumed was still there.
- **Refreshing such a collection anyway now reports every note in it as missing**, because the label changed while the notes on disk still carry the old one. Nothing is deleted and no duplicate file is written, but the summary is wrong from end to end.
- **"No orphans" is still printed when nothing was checked.** If the step that works out which notes are missing fails, the failure is recorded in a channel the wizard never shows anyone, and the finish message says "No orphans." regardless. That is the exact thing the third attempt set out to fix, fixed everywhere except where the check does not run. The "notes left unchanged" warning has the same shape: it tells you to see the summary screen, and that screen is not drawn for it.

Behind those, the mapping-import window prints `[object Object]` in place of the error when generation fails there, so the one thing a person needs is the one thing withheld.
