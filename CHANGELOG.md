...
### Known defect, mostly closed: each import now gets its own folder, but an existing vault needs care (2026-08-28, amended 2026-08-29 and four times on 2026-08-30)
...
**The work to make it safe for a vault that already holds imports has now been attempted five times.** Each attempt was measured against a bar written down in advance rather than a checklist. The fifth attempt is the first to finish half of it, and the half it finished is the half every previous attempt was about.
...
**Everything about who owns an import is now settled, and for the first time it has been run end to end.** The first four attempts each tried to work out which collection an import belongs to, and each was defeated by a case the previous one had not thought of. The fifth attempt found out why: there was one place in the code that guessed, underneath all four, and each attempt had removed a copy of it from a screen rather than the original. It looked inside the destination folder and adopted whichever single collection was already there. Deleting a screen that asked you the question simply routed the unanswered question back down to it.

What that produces, all of it verified by 2,286 passing tests, eighteen deliberate sabotages that each turned their own tests red, and a full end-to-end run of 47 walkthroughs with no regression:

- **Each import gets its own folder** inside your configured output path, and your setting stays the parent.
- **A second framework always lands as a new collection.** Not usually, and not unless something matches: there is no longer any code path that can decide otherwise. Ownership is decided on screen, by you, and the importer carries out the decision.
...
**None of it is offered here, because one thing is still wrong, and it is the one that can cost you notes.**

- **A new collection that owns nothing can still take another one's notes.** Who owns an import is now answered correctly, and then ignored one layer further down. The lookup that decides where each note goes searches your whole vault and has no idea collections exist, so an import that owns nothing still finds every note carrying an identifier it produces, wherever it is and whoever it belongs to, and treats it as its own. On *replace* it renames those notes into its own folder, rewrites them, and marks them as its own. On the default *skip* it leaves them and writes only the new rows, splitting one framework across two folders. Missing notes are counted only against what the import itself owns, and it owned nothing, so both cases report none missing.
- **The check meant to protect you from that walks you into it.** Import the same framework twice without clicking anything. The ownership step correctly says the first import stays separate. A folder check then notices the destination is occupied and tells you to choose another folder for this import. Do exactly that, and the row above takes every note of the first import. There is no second warning, and the screen is still saying the two stay separate while it happens. A variant needs no typing at all: change your output folder setting between two imports of the same framework and the folder check never fires.
...
- **The bundled-example command still reports how many errors happened without showing any of them.** Developer tooling, and the rule was that no path is exempt.
