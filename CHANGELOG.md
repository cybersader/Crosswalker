...
### Known defect, mostly closed: each import now gets its own folder, but an existing vault needs care (2026-08-28, amended 2026-08-29 and three times on 2026-08-30)
...
**The work to make it safe for a vault that already holds imports has now been attempted four times, and is still not finished.** Each attempt was measured against a bar written down in advance rather than a checklist, and none of them has cleared it.

All four attempts rest on the same rule, which is the durable result of the work and is not in question: **refreshing a collection you already have never changes where that collection lives.** Its folder is read from the notes it already owns and then remembered, so a refresh has nothing to move and nothing to leave behind, and moving a framework somewhere else becomes something you ask for rather than something that happens to you. The code carrying that rule holds against the unit tests and, as of the third attempt, caused no end-to-end regression. **The fourth attempt has had no end-to-end run at all**, so nothing below is claimed to work in a real Obsidian session.

**The fourth attempt stopped trying to work out which collection an import belongs to.** The first three each guessed and were each defeated by a case the previous one had not thought of, so the guess was removed rather than sharpened again.

- **Nothing preselects a refresh any more.** The ownership step lists every collection in your vault, with its folder, how many notes it holds, and what produced it, and the first and default choice is *import as a new collection*. A refresh happens because you picked one.
- **Matching became a suggestion.** When exactly one collection looks like the source you chose, one line appears offering it: *"Looks like this one. Refresh it instead?"* It is a button. If nothing matches, or several do, there is no line. Crosswalker may suggest; it does not decide.
- **A collection's name for its notes is now fixed when the collection is created**, instead of being worked out again from the file on every import. A collection you imported before this change is given the name its notes already carry, the first time you refresh it.
- **Two consequences of that, both of which were blocking.** A collection imported before this change can be refreshed again, because the list shows every collection rather than only the matching ones. And refreshing it no longer reports every note in it as missing.
- **A missing-notes count that was never worked out now says so** rather than printing "No orphans." The finish message and the summary screen both say it was not checked.
- **The mapping-import window shows its errors** instead of printing `[object Object]`, and stays open on a run that failed instead of closing over them.

**What the review found next: the same defect, on the surfaces nobody had looked at.**

- **The mapping-import window still decides for you, and its default is to overwrite.** It is the window used for every pair of frameworks you map together. If one collection is already in the folder for that pair, it is adopted with no control on the screen and no click, and that window replaces rather than skips, so an earlier mapping is rewritten in place. This is exactly the behaviour just removed from the main import wizard, still present one window over. A folder shared by two mappings does not tell you who owns what is in it.
- **The wizard cannot tell "we have not looked yet" from "there is nothing there."** While your vault is still being indexed the ownership step draws no control at all, so there is nothing to click, and an import started in that window falls back to the older rule and adopts whichever collection shares the folder. It needs a particular sequence to hit, but the check that would prevent it is simply absent.
- **The bundled-example command has no ownership control at all**, and reports how many errors happened without showing any of them.
- **Two mappings can claim the same top-level note.** A second mapping import lands correctly in its own folder as a new collection, with nobody clicking anything, and still renames the first mapping's top note into its own folder, leaving the first without one. That is not a wrong guess about ownership. It is a write reaching across a boundary that ownership answered correctly, and it is untouched by all four attempts.
