...
### Known defect, mostly closed: each import now gets its own folder, but an existing vault needs care (2026-08-28, amended 2026-08-29, five times on 2026-08-30, and again on 2026-08-31)
...
**The work to make it safe for a vault that already holds imports has now been attempted seven times.** Each attempt was measured against a bar written down in advance rather than a checklist. Each has ended one level below the last, and at every level the thing that had to be removed was the same thing: something deciding a fact instead of reading one.

All seven attempts rest on the same rule, which is the durable result of the work and is not in question: **refreshing a collection you already have never changes where that collection lives.** Its folder is read from the notes it already owns and then remembered, so a refresh has nothing to move and nothing to leave behind, and moving a framework somewhere else becomes something you ask for rather than something that happens to you.

**Who owns an import is settled and end-to-end verified, and so now is what a write is allowed to touch.** The first four attempts each tried to work out which collection an import belongs to and were each defeated by a case the previous one had not thought of; the fifth found the one place underneath all four that was guessing and deleted it. The sixth and seventh went lower again, to the part of the importer that places each note, which never consulted collections at all: it searched the whole vault for the identifier it was about to write, and, failing that, used whatever happened to be sitting in the spot it was aiming at. Both are now closed, and that is the end of the descent, because placing a note asks nothing else. What still blocks is no longer one level down. It is one level out: the same rules are written once per window rather than once, and one window that writes notes never had them at all.

What is true today, verified by deliberately sabotaging each rule below in turn and watching its own tests fail, and by a full end-to-end run of 47 walkthroughs in which sixteen previously failing checks turned green and none turned red:
...
**None of it is offered here, because the same rules are written once per window instead of once, and one window that writes notes was never given them.**

- **Linking a control to a piece of evidence can still replace a note outright.** That window writes a small note whose location comes from the two names, and if anything is already sitting there it is overwritten in full: nothing reads who owns it, nothing preserves your edits, nothing reports a conflict. This is the code the importer had before this work, on a window nobody had looked at. The folder it writes into is a setting, so pointing it at a folder that already holds notes is enough to reach it, and it then tells you it *updated the existing link* for a note that was never a link. This is the one remaining way to lose a note.
- **Two of the three windows that create a collection get the qualification rule wrong.** The import window is right. The bundled-example command never asks the question, so loading the same sample twice produces a page of refusals: safe, and not what anyone wants to see. The mapping window asks whether the destination folder is empty and treats that as an answer about identifiers, so moving that folder in Obsidian is enough to make the next import of the same pair collide with the one you moved. Both have been wrong since they were written.
- **A note whose properties are damaged is reported as not being Crosswalker's.** The ownership check has three answers and none of them is *I could not tell*, so a note the import genuinely owns, but cannot be read, is described as somebody else's and you are told to move or rename it. Nothing is written and nothing is lost, but the reason given is wrong. Two tests correctly fail on this and were left failing rather than adjusted.
...
- **The bundled-example command still reports how many errors happened without showing any of them.** It is the one path left that closes over its own errors.
- **Neither suite is green on this work.** Two unit tests fail, and they are the damaged-properties reason above. The end-to-end suite recovered sixteen walkthroughs with none lost, and one deterministic failure remains outside the long-standing quarantine: an internal test entry point importing twice without naming a collection, which is precisely what the new rule refuses. A red suite is not a verified one.
