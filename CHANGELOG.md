...
### Known defect, mostly closed: each import now gets its own folder, but an existing vault needs care (2026-08-28, amended 2026-08-29, five times on 2026-08-30, and twice on 2026-08-31)
...
- **A note whose properties are damaged is asked to be fixed, not disowned.** If Crosswalker cannot read a note's properties, it now says so and asks you to repair the note and import again. It no longer tells you the note is not Crosswalker's, and no longer asks you to move or rename your own imported note. Nothing is written for it either way, and the run continues.
...
**None of it is offered here, because the identity of an evidence link is still worked out from two file names rather than read off the notes, and that is the arc's own defect on a window.**

- **Linking evidence to two releases of the same control can still replace the first link.** The name of an evidence link is computed from the two file names, so two controls that share a filename — which is exactly what two releases of one framework look like — compute the same name. The window's new ownership check then sees a matching name, concludes it is the same link, and replaces it: an approved attestation, its reviewer, its date, its review baseline, and your notes, with the notice reporting an update. The check is right and the thing it checks is not an identity. This is the one remaining way to lose a note.
- **Two notes can end up claiming to be the same link.** If you drag an evidence link somewhere else in Obsidian, the next link for that pair finds nothing where it expected one and writes a second note carrying the same identifier. From then on every import reports an ambiguous identifier it cannot resolve, and no screen explains where it came from. The same thing happens for two controls whose filenames differ only by punctuation.
- **The one qualification rule can still read an unfinished index as an empty vault.** The rule now has a single implementation, but the two command-palette paths that use it do not wait for your vault to finish indexing first. Run the mapping import or the bundled example during startup on a large vault and it can create a collection whose identifiers collide with one it could not see yet, and then refuse every row of that import.
- **Re-baselining a reviewed link writes an audit fact to a location rather than to a note.** The housekeeping re-baseline picks its notes out of a previously generated report and writes the new review baseline to whatever note now sits at each of those locations, without checking that it is the same link. It is the only place in the plugin that records an attestation fact, and it records it by address.
...
- **The unit suite is green and the end-to-end suite is green outside its long-standing quarantine.** Two unit tests that were correctly failing on the damaged-properties reason now pass without being weakened, 41 new tests cover this pass, and the walkthrough that was failing outside the quarantine now passes. Eight quarantined failures remain, unchanged and reported separately. The suites are no longer what blocks; the four items above are.
